...
bool context::bcp() {
    ...
    while (m_qhead < m_assigned_literals.size()) {  // 遍历已赋值文字
        if (get_cancel_flag()) {
            return true;
        }
        literal l = m_assigned_literals[m_qhead];  // 获取一个已赋值的文字
        SASSERT(get_assignment(l) == l_true);      // 确保该文字是已经被断言为真的
        m_qhead++;
        m_simp_counter--;
        literal not_l = ~l;                         // 将该文字取反
        SASSERT(get_assignment(not_l) == l_false);  // 确保取反后的文字为假
        watch_list& w = m_watches[l.index()];       // 获取文字 l 的观察列表
        if (binary_clause_opt_enabled()) {          // 二元子句传播
            // binary clause propagation
            b_justification js(l);  // 导致下面传播的原因
            literal* it = w.begin_literals();
            literal* end = w.end_literals();
            for (; it != end; ++it) {  // 遍历二元子句中的另一个文字
                literal l = *it;       // 获取观察文字
                switch (get_assignment(l)) {
                    case l_false:                          // 若另一个文字为假
                        m_stats.m_num_bin_propagations++;  // 统计二元文字传播次数
                        set_conflict(js, ~l);              // 设置冲突
                        return false;
                    case l_undef:                          // 若另一个文字未赋值
                        m_stats.m_num_bin_propagations++;  // 统计二元文字传播次数
                        assign_core(l, js);                // 为其赋值
                        break;
                    case l_true:
                        break;
                }
        ...
        for (; it != end; ++it) {  // 遍历含有文字 l 的子句列表
            ...
            SASSERT(cls->get_literal(0) == not_l || cls->get_literal(1) == not_l);  // not_l 一定是左右观察文字之一
            if (cls->get_literal(0) == not_l) {                                     // 左观察文字若为假
                cls->set_literal(0, cls->get_literal(1));                           // 更新左观察文字为右观察文字
                cls->set_literal(1, not_l);                                         // 右观察文字更新为左观察文字
            }

            SASSERT(cls->get_literal(1) == not_l);  // 确保一定是右观察文字为假

            literal first_lit = cls->get_literal(0);          // 获取左观察文字
            lbool first_lit_val = get_assignment(first_lit);  // 获取左观察文字的取值

            if (first_lit_val == l_true) {  // 如果左观察文字为真
                *it2 = *it;                 // clause is already satisfied, keep it
                it2++;
            } else {
                literal* it3 = cls->begin() + 2;
                literal* end3 = cls->end();
                for (; it3 != end3; ++it3) {  // 遍历子句中其他文字（除左右观察文字外）
...
void context::remove_parents_from_cg_table(enode* r1) {
    ...
    for (enode* parent : enode::parents(r1)) {
        ...
        CTRACE(
            "add_eq", !parent->is_marked() && parent->is_cgc_enabled() && !parent->is_true_eq() && parent->is_cgr() && !m_cg_table.contains_ptr(parent),
...
bool context::propagate() {
    ...
    while (true) {
        ...
        unsigned qhead = m_qhead;
        {
            ...
            if (!bcp()) {
                // puts("propagating bcp");
                return false;
            }
            if (!propagate_th_case_split(qhead)) {
                // puts("propagating propagate_th_case_split");
                return false;
            }
            SASSERT(!inconsistent());
            ...
            if (inconsistent())
                return false;
            if (!propagate_atoms()) {
                // puts("propagating propagate_atoms");
                return false;
            }

            if (!propagate_eqs()) {
                // puts("propagating propagate_eqs");
                return false;
            }
            ...
            if (!propagate_theories()) {
                // puts("propagating propagate_theories");
                return false;
            }
        }
        if (!get_cancel_flag()) {
...
bool context::guess(bool_var var, lbool phase) {
    ...
    m_fparams.m_phase_selection = PS_RANDOM; // change
...
bool context::decide() {
    ...
    TRACE("decide_var", tout << "berfore decide_var: " << var << "\n");

    if (!has_split_candidate(var, is_pos)) {  // 若不存在候选决策变量
        lbool phase = l_undef;
        m_case_split_queue->next_case_split(var, phase); // my change
        used_queue = true;
        ...
        is_pos = guess(var, phase);  // my change 
        // puts("guess there ");
    }

    TRACE("decide_var", tout << "after decide_var: " << var << "\n");
...
void context::mk_th_case_split(unsigned num_lits, literal* lits) {
    ...
    if (!m_fparams.m_theory_case_split) {
        ...
        TRACE(
            "theory_case_split", tout << "tracking case split literal set { ";
            for (unsigned i = 0; i < num_lits; ++i) {
                tout << lits[i].index() << " ";
            } tout
            << "}" << std::endl;);
...
bool context::propagate_th_case_split(unsigned qhead) {
    ...
    for (; qhead < assigned_literal_end; ++qhead) {
        ...
        for (vector<literal_vector>::const_iterator it = case_split_sets.begin(); it != case_split_sets.end(); ++it) {
            ...
            TRACE(
                "theory_case_split", tout << "found case split set { ";
...
final_check_status context::final_check() {
    ...
    do {
        ...
        if (m_final_check_idx < num_th) {
            ...
            TRACE("final_check_step", tout << "quantifier  ok: " << ok << " "
...
bool context::resolve_conflict() {
    ...
    if (m_conflict_resolution->resolve(m_conflict, m_not_l)) {
        ...
        TRACE(
            "resolve_conflict_bug",
            tout << "AFTER m_scope_lvl: " << m_scope_lvl << ", new_lvl: " << new_lvl << ", lemma_intern_lvl: " << m_conflict_resolution->get_lemma_intern_lvl() << "\n";
            tout << "num_lits: " << num_lits << "\n";
            for (unsigned i = 0; i < num_lits; i++) {
                literal l = lits[i];
                tout << l << " ";
